...
generate_latex_table1 <- function(modelsfilepath, tablename) {
  ...
  p_obj <- get(ls(pattern = "^lrqap_pos_models")[1])
  n_obj <- get(ls(pattern = "^lrqap_neg_models")[1])
  
  model_list <- list(
    pos_all = p_obj$`All A`,
    pos_pol = p_obj$`Political A`,
    pos_ent = p_obj$`Entertainment A`,
    
    neg_all = n_obj$`All A`,
    neg_pol = n_obj$`Political A`,
    neg_ent = n_obj$`Entertainment A`
  )
...
generate_latex_table1("results/new_qap_results-adfontes.RData", "tables/table1R-adfontes.tex")

generate_latex_table1("results/new_qap_results-10seasons.RData", "tables/table1R-10seasons.tex")

generate_latex_table1("results/new_qap_results-placebo.RData", "tables/table1R-placebo.tex")
