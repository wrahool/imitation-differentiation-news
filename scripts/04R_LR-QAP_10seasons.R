...
# This script does a robustness test using 10 seasons instead of 60
...
res <- read_csv("auxiliary/inter-media-influence-entertainment_10seasons.csv")
...
# Even when controlling for the structural tendency of media outlets to influence each other reciprocally, increasing ideological distance significantly drives influence in the other direction ($\beta = -0.0046, p < 0.05$)
...
# Without controlling for the structural tendency of media outlets to influence each other reciprocally, increasing ideological distance significantly drives influence in the other direction ($\beta = -0.01, p < 0.05$)
...
# Effect is negative (-0.31) but not significant (p = 0.172)
# Exp(b) is 0.74
...
# Effect is negative (-0.44) but not significant (p = 0.219)
# Exp(b) is 0.64
...
qap_cor_models_10seasons <- list("All A" = m1_all,
                       "All B" = m2_all,
                       "Political A" =  m1_pol,
                       "Political B" = m2_pol,
                       "Entertainment A" = m1_ent,
                       "Entertainment B" = m2_ent)

# Combine all positive influence LRQAP models into a list
lrqap_pos_models_10seasons <- list("All A" = logit_pos_allA,
                         "All B" = logit_pos_allB,
                         "Political A" = logit_pos_polA,
                         "Political B" = logit_pos_polB,
                         "Entertainment A" = logit_pos_entA,
                         "Entertainment B" = logit_pos_entB)

# Combine all negative influence LRQAP models into a list
lrqap_neg_models_10seasons <- list("All A" = logit_neg_allA,
                         "All B" = logit_neg_allB,
                         "Political A" = logit_neg_polA,
                         "Political B" = logit_neg_polB,
                         "Entertainment A" = logit_neg_entA,
                         "Entertainment B" = logit_neg_entB)

save(qap_cor_models_10seasons,
     lrqap_pos_models_10seasons,
     lrqap_neg_models_10seasons,
     file = "results/new_qap_results-10seasons.RData")
